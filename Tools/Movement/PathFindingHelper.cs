...
namespace Tools.Movement
{
    class PathFindingHelper
    {
        ...
        public PathFindingHelper(IMonitor monitor, MovementConfig config)
        {
        ...
        public void updateLineToTiles(Vector2 start, Vector2 destination, bool flushAllDrawedNode = true)
        {
            ...
            List<Vector2> line = lineToTiles(TileUtil.toTile(start), TileUtil.toTile(destination));
            for (var i = 0; i < line.Count; i += 1)
            {
                var tile = TileUtil.fixFragtionTile(line[i]);
                var node = new DrawableNode(TileUtil.toBoxPosition(tile));
                if (i >= 1)
                {
                    var prev = TileUtil.fixFragtionTile(line[i - 1]);
        ...
        public void tryDirectMove(Vector2 start, Vector2 destination, bool flushAllDrawedNode = true)
        {
            ...
            for (var i = 0; i < line.Count; i += 1)
            {
                path.Add(TileUtil.toPosition(line[i]));
            }
        ...
        public void drawIndicator(SpriteBatch b)
        {
            ...
            if (TileUtil.debugPassable)
                TileUtil.drawPassable(b);
        ...
        public void changeDes(Vector2 destination)
        {
            if (this.debugVerbose)
            {
                this.Monitor.Log(String.Format("Change destination from {0} to {1}, tile value {2} to {3}", this.destination, destination, TileUtil.toTile(this.destination), TileUtil.toTile(destination)), LogLevel.Info);
            }
            this.originalDestination = destination;

            this.flushCache();
            TileUtil.flushCache();
            targetNode = new DrawableNode(TileUtil.toBoxPosition(destination));
            currentStep = 0;
            if (Config.EnablePathFinding)
                aStarPathFinding(destination);
            else
                tryDirectMove(TileUtil.toPosition(Game1.player.Tile), destination);
        }
        ...
        {
            return TileUtil.toTile(this.destination);
        }
        ...
        private bool isValidMovement(Vector2 current, Vector2 neighbor)
        {
            ...
            // tile isn't passable 
            if (!TileUtil.isTilePassable(neighbor) || !TileUtil.isTilePassable(current))
                return false;

            // diagonal special case handling, just don't do it if there is a blockage
            if ((i == 1 || i == -1) && (j == 1 || j == -1))
            {
                if (!TileUtil.isTilePassable(current.X, neighbor.Y) || !TileUtil.isTilePassable(neighbor.X, current.Y))
                {
                    return false;
                }
            }
            ...
            if (Game1.player.isRidingHorse())
            {
                ...
                if (!TileUtil.isTilePassable(neighbor.X - 1, neighbor.Y))
                {
                    checkBlockage = checkBlockage || (!TileUtil.isTilePassable(neighbor.X + 1, neighbor.Y - 1));
                    checkBlockage = checkBlockage || (!TileUtil.isTilePassable(neighbor.X + 1, neighbor.Y));
                    checkBlockage = checkBlockage || (!TileUtil.isTilePassable(neighbor.X + 1, neighbor.Y + 1));
                }
        ...
        public void aStarPathFinding(Vector2 destination)
        {
            ...
            this.destinationTile = TileUtil.toTile(this.destination);
            ...
            float bestScore = Vector2.Distance(TileUtil.toPosition(start), this.destination);
            ...
            int limit = Config.PathFindLimit;
            while (pq.Count > 0 && limit > 0)
            {
                ...
                if (debugVerbose)
                {
                    this.Monitor.Log(String.Format("[Step {2}] Current {3} or in tile {0} = {1} fScore {4}", current, gScore[current], 100 - limit, addPadding(current), fScore[current]), LogLevel.Info);
                }
                ...
                foreach (var neighbor in neighborList)
                {
                    ...
                    this.visitedNodes.Add(new DrawableNode(TileUtil.toBoxPosition(neighbor)));

                    var temp = gScore[current] + calculateGScore(TileUtil.toPosition(current), TileUtil.toPosition(neighbor));
                    if (gScore.ContainsKey(neighbor))
                    {
                        ...
                        else
                        {
                            ...
                            fScore[neighbor] = temp + calculateFScore(TileUtil.toPosition(neighbor));
                        }
                    }
                    else
                    {
                        ...
                        fScore.Add(neighbor, temp + calculateFScore(TileUtil.toPosition(neighbor)));
                    }
                    ...
                    if (neighbor.X == this.destinationTile.X && neighbor.Y - this.destinationTile.Y >= 0 && neighbor.Y - this.destinationTile.Y < 1.1f)
                    {
                        if (!isBestScoreFront)
                        {
                            bestScore = Vector2.Distance(TileUtil.toPosition(neighbor), this.destination);
                        ...
                        if (isBestScoreFront && (bestScore > Vector2.Distance(TileUtil.toPosition(neighbor), this.destination)))
                        {
                            bestScore = Vector2.Distance(TileUtil.toPosition(neighbor), this.destination);
                            bestScoreTile = neighbor;
                            if (this.debugVerbose)
                        ...
                        if (!TileUtil.isTilePassable(this.destinationTile) && bestScore < 80f)
                        {
                    ...
                    else
                    {
                        if (!isBestScoreFront && (bestScore > Vector2.Distance(TileUtil.toPosition(neighbor), this.destination)))
                        {
                            ...
                            if (!TileUtil.isTilePassable(this.destinationTile + new Vector2(0, 1)) && !TileUtil.isTilePassable(this.destinationTile) && bestScore < 80f)
                            {
                                this.destinationTile = bestScoreTile;
                                updatePath(tracebackAndUpscalePath());
                                return;
        ...
        public List<Vector2> tracebackAndUpscalePath()
        {
            ...
            if (this.cameFrom.ContainsKey(TileUtil.toTile(this.destination)))
            {
                if (debugVerbose)
                    this.Monitor.Log(String.Format("Path traceback (start): {0} or in tile {1}", this.destination, TileUtil.toTile(this.destination)));
            ...
            while (Vector2.Distance(pointerTile, startTile) > this.microTileDelta)
            {
                ...
                if (debugVerbose)
                    this.Monitor.Log(String.Format("Path traceback (loop): {0} or in tile {1}", traceBackPosition, pointerTile));
            ...
            if (debugVerbose)
                this.Monitor.Log(String.Format("Path traceback (end): {0} or in tile {1}", temp.Peek(), startTile));
        ...
        public List<Vector2> populateSmothPath(List<Vector2> smothPath)
        {
            ...
            if (smothPath.Count > 1)
                ...
                {
                    var start = TileUtil.toTile(smothPath[i - 1]);
                    var end = TileUtil.toTile(smothPath[i]);
                    foreach (var tile in lineToTiles(start, end, skipStartTile: i == 0))
                    {
                        var fixTile = TileUtil.fixFragtionTile(tile);

                        if (TileUtil.isTilePassable(fixTile))
        ...
        public void updatePath(List<Vector2> path)
        {
            ...
            for (var i = 0; i < path.Count; i += 1)
            {
                ...
                if (this.debugVerbose)
                {
                    this.Monitor.Log(String.Format("Path: {0} of in tile {1}", path[i], TileUtil.toTile(path[i])), LogLevel.Info);
                }
            }
        ...
        public Vector2 addPadding(Vector2 positionTile)
        {
            ...
            bool checkLeft = !TileUtil.isTilePassable(positionTile.X - 1, positionTile.Y);
            checkLeft = checkLeft || !TileUtil.isTilePassable(positionTile.X - 1, positionTile.Y - 1);
            checkLeft = checkLeft || !TileUtil.isTilePassable(positionTile.X - 1, positionTile.Y + 1);

            // blockage on right side by any mean
            bool checkRight = !TileUtil.isTilePassable(positionTile.X + 1, positionTile.Y);
            checkRight = checkRight || !TileUtil.isTilePassable(positionTile.X + 1, positionTile.Y - 1);
            checkRight = checkRight || !TileUtil.isTilePassable(positionTile.X + 1, positionTile.Y + 1);
            ...
            bool checkTop = !TileUtil.isTilePassable(positionTile.X - 1, positionTile.Y - 1);
            checkTop = checkTop || !TileUtil.isTilePassable(positionTile.X, positionTile.Y - 1);
            checkTop = checkTop || !TileUtil.isTilePassable(positionTile.X + 1, positionTile.Y - 1);

            // blockage on bottom side by any mean
            bool checkBottom = !TileUtil.isTilePassable(positionTile.X - 1, positionTile.Y + 1);
            checkBottom = checkBottom || !TileUtil.isTilePassable(positionTile.X, positionTile.Y + 1);
            checkBottom = checkBottom || !TileUtil.isTilePassable(positionTile.X + 1, positionTile.Y + 1);
            ...
            var res = TileUtil.toPosition(positionTile, padX, padY);
            return res;
        }
        ...
        {
            Vector2 startTile = TileUtil.toTile(start);
            Vector2 endTile = TileUtil.toTile(end);
            ...
            {
                var tile = TileUtil.fixFragtionTile(lineTiles[i]);
                if (i > 0)
                {
                    var prev = TileUtil.fixFragtionTile(lineTiles[i - 1]);
        ...
        public Vector2 moveDirection()
        {
            ...
            if (TileUtil.isTilePassable(TileUtil.toTile(this.destination)))
            {
                // and we found the path lead to it
                if (TileUtil.toTile(this.destination) == this.destinationTile)
                {
            ...
            if (optionalNext is null)
            {
                // Note: checkColidingIfMoving was in ModEntry, we might need to handle this differently
                // For now, let's just return direction and let collision happen
                return direction; 
            }
