    ...
    public class TileUtil
    {
        ...
        private static bool _isTilePassableNew(Vector2 tile)
        {
            ...
            if (building is not null)
            {
                ...
                {
                    if (TileUtil.debugPassable)
                        nonPassableNodes.Add(new DrawableNode(TileUtil.toBoxPosition(tile)));
                    if (TileUtil.debugPassableVebose)
                        _monitor?.Log("Found unpassable building " + building + " at tile " + tile, LogLevel.Info);
                    return false;
                }
            }
        ...
        private static bool _isTilePassableOld(Vector2 tile)
        {
            ...
            {
                if (TileUtil.debugPassableVebose)
                    _monitor?.Log("Found unpassable tile from current location at " + tile, LogLevel.Info);
            ...
            foreach (var items in gl.terrainFeatures)
            {
                ...
                if (TileUtil.debugPassableVebose)
                    _monitor?.Log("Found unpassable terrain feature " + items[tile], LogLevel.Info);
                cacheCantPassable.Add(tile);
                if (TileUtil.debugPassable)
                    nonPassableNodes.Add(new DrawableNode(TileUtil.toBoxPosition(tile)));
                return false;
            ...
            if (gl.Objects.ContainsKey(tile))
            {
                ...
                if (!item.isPassable())
                {
                    ...
                    if (TileUtil.debugPassableVebose)
                        _monitor?.Log("Found unpassable object" + item, LogLevel.Info);
                    cacheCantPassable.Add(tile);
                    if (TileUtil.debugPassable)
                        nonPassableNodes.Add(new DrawableNode(TileUtil.toBoxPosition(tile)));
                    return false;
                    ...
                    if (bed.bedType != BedFurniture.BedType.Child)
                    {
                        ...
                        if (tile.X <= bedSpot.X + bed.getTilesWide() & tile.X >= bedSpot.X & tile.Y > bedSpot.Y & tile.Y < bedSpot.Y + 2)
                        {
                            if (debugPassableVebose)
                                _monitor?.Log("Found unpassable furniture" + funiture, LogLevel.Info);
                            return true;
                        }
                    }
                }
