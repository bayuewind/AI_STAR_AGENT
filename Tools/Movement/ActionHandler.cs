...
namespace Tools.Movement
{
    ...
    public class ActionHandler : IActionHandler
    {
        ...
        public ActionHandler(IMonitor monitor, bool isVerbose = false)
        {
        ...
        public void debugDoAction(Vector2 target)
        {
            ...
            if (this.isVerbose)
            {
                Monitor.Log(String.Format("Try do action at tile {0}, full context: {1}", TileUtil.toTile(target), this.toString()), LogLevel.Trace);
            }
            tryDoAction();
            cancelAction();
        }
        ...
        public bool tryDoAction()
        {
            ...
            var grabTile = TileUtil.toTile(this.target);
            ...
            if (isTryToDoActionAtClickedTitle == 1 && this.targetObject != null && (Utility.tileWithinRadiusOfPlayer((int)grabTile.X, (int)grabTile.Y, 1, Game1.player) || Utility.tileWithinRadiusOfPlayer((int)targetObject.TileLocation.X, (int)targetObject.TileLocation.Y, 1, Game1.player)))
            {
                ...
                {
                    if (this.isVerbose)
                        Monitor.Log(String.Format("Check object directly instead {0}", targetObject.TileLocation));
                    isTryToDoActionAtClickedTitle = 0;
                    //isMovingAutomaticaly = false;
                    return true;
                }
                else
                {
                    ...
                    {
                        if (this.isVerbose)
                            Monitor.Log(String.Format("Got Chest type at {0}, we do special handling", targetObject.TileLocation));
                ...
                if (this.isVerbose)
                    Monitor.Log(String.Format("Got Special object that return false when try to check!"));
                isTryToDoActionAtClickedTitle = 0;
                //isMovingAutomaticaly = false;
                return true;
            }
            ...
            {
                if (this.isVerbose)
                    Monitor.Log(String.Format("Got TerrainFeatures type: {0}, we do special handling at grapTile: {1}", targetTerrainFeatures, grabTile));
                isTryToDoActionAtClickedTitle = 0;
                return true;
            }
            ...
            {
                if (this.isVerbose)
                    Monitor.Log(String.Format("Got LargeTerrainFeatures type: {0}, we do special handling at grapTile: {1}", targetLargeTerrainFeatures, grabTile));
                isTryToDoActionAtClickedTitle = 0;
                return true;
            }
            ...
            if (isTryToDoActionAtClickedTitle == 4 && Utility.tileWithinRadiusOfPlayer((int)grabTile.X, (int)grabTile.Y, 1, Game1.player) && targetBuilding != null)
            {
                ...
                {
                    if (this.isVerbose)
                        Monitor.Log(String.Format("Got Building type: {0}, we do special handling at grapTile: {1}", targetBuilding, grabTile));
                    isTryToDoActionAtClickedTitle = 0;
                    return true;
                }
            ...
            if (isTryToDoActionAtClickedTitle == 4 && Utility.tileWithinRadiusOfPlayer((int)grabTile.X, (int)grabTile.Y, 1, Game1.player))
            {
                ...
                {
                    if (this.isVerbose)
                        Monitor.Log(String.Format("Just checked at {0}", grabTile), LogLevel.Trace);
                }
                else
                {
                    if (this.isVerbose)
                        Monitor.Log(String.Format("Just failed checked at {0}", grabTile), LogLevel.Trace);
                }
                isTryToDoActionAtClickedTitle = 0;
                return true;
            }
        ...
        {
            this.target = target;
            isTryToDoActionAtClickedTitle = GetActionType(TileUtil.toTile(this.target));
        }
