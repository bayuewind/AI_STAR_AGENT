...
public class CropMap
{
    ...
    private Dictionary<XnaVector2, List<Tile>> neighborDict;
    private Dictionary<XnaVector2, List<Tile>> coveredByDict;

    public CropMap()
    {
        this.map = new List<List<Tile>>();
        this.waterableTiles = new List<Tile>();
        this.neighborDict = new Dictionary<XnaVector2, List<Tile>>();
        this.coveredByDict = new Dictionary<XnaVector2, List<Tile>>();
    ...
    /// </summary>
    public static XnaPoint convertCoords(int x, int y, int width, int height)
    {
        return new XnaPoint(y, x);
    }
    ...
    {
        return Game1.currentLocation.isCollidingPosition(new XnaRectangle(y * 64 + 1, x * 64 + 1, 62, 62), Game1.viewport, isFarmer: true, -1, glider: false, Game1.player);
    }
    ...
    {
        XnaVector2 index = new XnaVector2(x, y);
    ...
    public void loadMap()
    {
        ...
        for (int y = 0; y < this.height; y++)
        {
            ...
            {
                XnaPoint converted = CropMap.convertCoords(x, y, this.width, this.height);
                bool needsWatering = CropMap.tileNeedsWatering(converted.Y, converted.X);

                Tile tile = new Tile(
                    x,
                    y,
                    CropMap.tileIsPassable(converted.X, converted.Y),
                    CropMap.tileIsRefillable(converted.X, converted.Y),
                    needsWatering
                );
    ...
    /// <summary>
    /// Find all groupings of waterable tiles using minimal cover algorithm.
    /// </summary>
    public List<Group> getMinimalCoverGroups()
    {
        ...
        {
            var waterableVector = new XnaVector2(waterableTile.x, waterableTile.y);
            ...
            foreach (var change in directions.Concat(diagonals))
            {
                if (change.IsInBounds(waterableTile.y + change.Y, waterableTile.x + change.X))
                {
                    ...
                    {
                        var neighborVector = new XnaVector2(neighborTile.x, neighborTile.y);
        ...
        var lonelyTiles = waterableTiles.Where(tile => !coveredByDict.ContainsKey(new XnaVector2(tile.x, tile.y))).ToArray();
        foreach (var tile in lonelyTiles)
        {
            groups.Add(tile, new() { tile });
            neighborDict.Remove(new XnaVector2(tile.x, tile.y));
        }
        
        while (neighborDict.Count > 0)
        {
            var freedUpList = new List<XnaVector2>();
            foreach (var tilePair in neighborDict)
            {
                var tileVector = tilePair.Key;
                var neighbors = tilePair.Value;
                if (neighbors!.Count == 9) continue;
                var tile = neighbors.Find(neighbor => new XnaVector2(neighbor.x, neighbor.y) == tileVector);
                var isInShadow = neighbors.Any(neighbor => neighbor != tile && neighborDict.TryGetValue(new XnaVector2(neighbor.x, neighbor.y), out var featureList) && featureList.Intersect(neighbors).Count() == neighbors.Count);
                if (isInShadow || neighbors.Count == 0)
                {
                    foreach (var neighbor in neighbors)
                    {
                        var neighborVector = new XnaVector2(neighbor.x, neighbor.y);
            ...
            if (currentGroupCount == groups.Count)
            {
                ...
                if (currentGroupCount == groups.Count) break;
            }
        }
        return groups.Select((pair, i) => new Group(i, pair.Key, pair.Value)).ToList();
    }

    private void pickUniques(Dictionary<Tile, List<Tile>> groups, List<XnaVector2> freedUpList, int threshold = 1)
    {
        foreach (var freedUp in freedUpList)
        {
            ...
            if (foundCover && covers.Count == threshold && !neighborDict.ContainsKey(freedUp))
            {
                ...
                {
                    var waterableVector = new XnaVector2(groupCenter.x, groupCenter.y);
    ...
    private void removeAllAsNeighbors(List<Tile> tiles)
    {
        ...
        {
            var waterableVector = new XnaVector2(tile.x, tile.y);
            if (coveredByDict.TryGetValue(waterableVector, out var coverteeList))
            {
                foreach (var covertee in coverteeList)
                {
                    if (neighborDict.TryGetValue(new XnaVector2(covertee.x, covertee.y), out var coverteeNeighbors))
                    {
                        neighborDict[new XnaVector2(covertee.x, covertee.y)] = coverteeNeighbors.Where(x => x != tile).ToList();
                    }
                }
            }
        }
    ...
    /// <summary>
    /// Find all groupings of waterable tiles using DFS.
    /// </summary>
    public List<Group> findGroupings(ConsoleLog console)
    {
    ...
    /// </summary>
    public Tuple<List<Tile>, int> walkablePathBetweenPoints(ConsoleLog console, XnaPoint start, XnaPoint end)
    {
    ...
    /// </summary>
    public int[,] generateCostMatrix(ConsoleLog console, List<Group> groupings)
    {
        ...
        for (int i = 0; i < nodes; i++)
        {
            for (int j = 0; j < nodes; j++)
            {
                ...
                else
                {
                    ...
                    {
                        XnaPoint start = new XnaPoint(Game1.player.TilePoint.X, Game1.player.TilePoint.Y);
                        XnaPoint end = groupings[i == 0 ? j - 1 : i - 1].Centroid(this);
                    ...
                    {
                        XnaPoint start = groupings[i - 1].Centroid(this);
                        XnaPoint end = groupings[j - 1].Centroid(this);
    ...
    /// </summary>
    public List<Group> findGroupPath(ConsoleLog console, List<Group> groupings)
    {
        try
        {
    ...
    /// </summary>
    public List<ActionableTile> findFillPath(Group group, ConsoleLog console)
    {
    ...
    /// </summary>
    public ActionableTile? getClosestRefill(Tile start, ConsoleLog console)
    {
        foreach (List<Tile> row in this.map)
        {
            foreach (Tile tile in row)
        ...
        List<Tuple<Tile, Tile>> queue = new() { new(start, null!) };
        ...
        while (queue.Count > 0)
        {
            ...
            if (current.water)
            {
                List<XnaPoint> actions = new() { current.getPoint() };
